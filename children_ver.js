    ...
    function fillCalendar(calendar){
        ...
        clearStyles();
        for(var i = 3; i < 9; i++){
            var row = calendar.children[0].children[i];
            for (let j = 0; j < 7; j++) {

                row.children[j].innerHTML = d.getDate();
                if (d.getMonth() != month)
                    row.children[j].classList.add("hidden-days");
                if (d.getTime() == today)
                    row.children[j].classList.add("today");
                if(j > 4)
                    row.children[j].classList.add("holydays");
    ...
    function clearStyles(){
        for(var i = 3; i < 9; i++){
            var row = calendar.children[0].children[i];
            for (var j = 0; j < 7; j++){
                row.children[j].classList.remove("hidden-days", "today");
                if (j > 4)
                row.children[j].classList.remove("hidden-days", "holydays");
            }
        }
    }
