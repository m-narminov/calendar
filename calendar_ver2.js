	...
	day = (6 + day) % 7 + 1;
		...
		if(d.getMonth() != month) calendar.tBodies[0].rows[i].cells[j].classList.add("hidden-days");
                if(d.getTime() == today) calendar.tBodies[0].rows[i].cells[j].classList.add("today");
                if(j > 4) calendar.tBodies[0].rows[i].cells[j].classList.add("holydays");
    ...
    function clearStyles(){
        for(var i = 3; i < 9; i++){
            for (var j = 0; j < 7; j++){
                ...
                if (j > 4) calendar.tBodies[0].rows[i].cells[j].classList.remove("hidden-days", "holydays");
            }
        }
    }
    ...
    if (prevM) prevM.addEventListener("click", setPrevMonth, true);
    if (nextM) nextM.addEventListener("click", setNextMonth, true);
    showCalendar();
